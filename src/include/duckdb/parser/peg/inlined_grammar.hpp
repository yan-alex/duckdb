...
const char INLINED_PEG_GRAMMAR[] = {
	...
	"# LEVEL 3\n"
	"LogicalNotExpression <- SparkNotExpression* IsExpression\n"
	...
	"BetweenInLikeOp <- 'NOT'? (BetweenClause / InClause / LikeAnyAllClause / LikeClause)\n"
	...
	"PostfixOperator <- '!'\n"
	"SpecialFunctionExpression <- CoalesceExpression / UnpackExpression / TryExpression / ColumnsExpression / ExtractExpression / TimestampDiffExpression / LambdaExpression / NullIfExpression / PositionExpression / RowExpression / SubstringExpression / TrimExpression\n"
	...
	"PositionExpression <- 'POSITION' Parens(SingleExpression PositionSeparator SingleExpression)\n"
	...
	"CreateViewStmt <- 'RECURSIVE'? 'VIEW' IfNotExists? QualifiedName ViewColumnList? WithSchemaMode? WithList? 'AS' SelectStatementInternal\n"
	...
	"CreateTableAs <- SparkUsing? IdentifierList? PartitionSortedOptions? WithList? 'AS' Statement WithData?\n"
	...
	"CreateColumnList <- Parens(CreateTableColumnList?) SparkUsing? PartitionSortedOptions? WithList?\n"
	...
	"DropSchema <- ('SCHEMA' / 'DATABASE') IfExists? List(QualifiedSchemaName)\n"
	...
	"ValuesRef <- (ValuesClause / ValuesClauseNoParens) TableAlias?\n"
	...
	"ValuesClauseWithAlias <- (ValuesClause / ValuesClauseNoParens) TableAlias\n"
